...
- 正規分布の確率密度関数から確率を計算する方法
  - 体長が4センチ以上5センチ以下となる確率は
  - 体長の確率密度関数を4～5の間で積分する
  - 正規分布の場合は、これを直接計算するのが困難なので、
  - 「-∞～5積分値」-「-∞～4積分値」という計算をする
- 確率密度から期待値を計算する方法
  - 離散値と同じように「確率×その時の値」の合計値
  - 正規分布の場合、期待値は母数μと一致する

$$
\mu = \int_{-\infty}^\infty N(x|\mu,\sigma^2) \cdot x dx
$$
